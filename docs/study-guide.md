...
- [Overview](https://www.palantir.com/docs/foundry/superrepo/overview/)
- [Core concepts](https://www.palantir.com/docs/foundry/superrepo/core-concepts/)
- [Create a SuperRepo](https://www.palantir.com/docs/foundry/superrepo/create-a-superrepo)
- [Foundry CLI](https://www.palantir.com/docs/foundry/superrepo/foundry-cli)
...
- [Install a Developer Console application with Marketplace](https://www.palantir.com/docs/foundry/developer-console/marketplace-installation/)
...
Local preview is a simulator. Local servers “[mock a subset of interactions with a real Foundry instance](https://www.palantir.com/docs/foundry/superrepo/core-concepts/).” Deploy is not “upload a website.” Deploy is “turn this repo into a Marketplace product and install it.”
...
flowchart TB
  ...
  marketplace -->|"object types, links, actions, functions"| ontology
  project --> devConsole
```
...
A Dev-tier license is one enrollment. Community threads (Palantir staff, not an official product-docs page) report enrollment-wide quotas around **60 object types, 60 one-to-many link types, and 60 action types**. Harbor Desk adds **seven** object types, **thirteen** one-to-many links, and **six** actions. Count before you install.

### 2.2 Project (Compass)

A **[Project](https://www.palantir.com/docs/foundry/getting-started/projects-and-resources/)** is a folder that owns resources: datasets, code repositories, Workshop applications, Developer Console applications, and Marketplace installations. When `foundry deploy configure` asks where to install, it is asking **which Project should own the [Compass](https://www.palantir.com/docs/foundry/compass/overview/) outputs** (and, on many enrollments, the ontology resource files). Palantir also prompts for namespace and website domain.

Marketplace install also asks which **Ontology** should receive object types, links, actions, and functions ([Install a product](https://www.palantir.com/docs/foundry/marketplace/install-product/)). The Ontology is chosen from the space. Object types, links, actions, and functions are created **in that Ontology**. The Developer Console website and the installation record live in the **Project**.

Those types are not magically visible to every user on the enrollment. Other apps can use them **if they have permission** (Ontology roles, project roles, markings). Where the ontology *resource files* live in Compass depends on the enrollment:

- December 2025: project-based permissions for ontology resources entered beta ([announcement](https://www.palantir.com/docs/foundry/announcements/2025-12/)).
- 21 January 2026: **new ontologies default** to saving ontology resources in Compass projects ([announcement](https://www.palantir.com/docs/foundry/announcements/2026-01/)).
- May 2026: **CBAC** enrollments can store ontology resources in Compass ([announcement](https://www.palantir.com/docs/foundry/announcements/2026-05/)).
- Still **not** available for Default Ontologies ([migrate to project-based permissions](https://www.palantir.com/docs/foundry/ontology-manager/migrate-to-project-based-permissions/)).

After that migration, Marketplace puts ontology resource files in the chosen Project alongside the rest of the product. Before it, those files land in the Ontology service project; non-ontology files still go to the Project you pick.

### 2.3 Ontology

The **ontology** is the data model: object types, properties, links, actions, functions, security. [Ontology Manager](https://www.palantir.com/docs/foundry/ontology-manager/overview/) is the UI for the same layer SuperRepo authors in `ontology.mts`.

After Harbor Desk deploys, `analyst`, `person`, `organization`, `ownershipInterest`, `wallet`, `investigationCase`, and `finding` are real object types. [Object Explorer](https://www.palantir.com/docs/foundry/object-explorer/overview) can list them. [Workshop](https://www.palantir.com/docs/foundry/workshop/overview) can be built on them. AIP can be granted them. They are not “SuperRepo-only objects.” “[Applications are never siloed by how their Ontology types were created](https://www.palantir.com/docs/foundry/superrepo/overview/).”
...
2. These types use `includeEmptyBackingDatasource: true`. They do not need a dataset ([continue without an existing datasource](https://www.palantir.com/docs/foundry/object-link-types/create-object-type/)). Instances exist because **actions** (or local seed) created them. That is different from a typical Foundry object type, which is backed by a dataset or stream produced by a pipeline. User edits still go through actions either way ([object edits](https://www.palantir.com/docs/foundry/object-edits/overview/)).
...
`foundry deploy` uploads the product version and installs it with the inputs in `env.yml` ([core concepts](https://www.palantir.com/docs/foundry/superrepo/core-concepts/)). A later `foundry deploy` of a new version is that same Marketplace install path with a new product version.

This product sets `installMode: SINGLETON`, which is Palantir’s default SuperRepo `foundry.yml` example. Palantir’s Developer Console Marketplace docs: a Singleton product is deployed **once per Ontology**, so API names stay unique ([Install a Developer Console application with Marketplace](https://www.palantir.com/docs/foundry/developer-console/marketplace-installation/)). SINGLETON is that once-per-Ontology constraint, not the mechanism that upgrades an existing install.

Why SuperRepo uses Marketplace: Palantir already had a way to ship ontology + functions + apps as one reproducible, [cryptographically signed](https://www.palantir.com/docs/foundry/superrepo/overview/) bundle. SuperRepo compiles down to that artifact so local git and Foundry CI can use the same path.

### 2.5 Developer Console

A custom React/OSDK website is a **[Developer Console](https://www.palantir.com/docs/foundry/developer-console/overview) application**: OAuth client, redirect URLs, hosted subdomain, uploaded website assets.
...
Pipelines and Data Connections **produce datasets** that normally **back** object types. SuperRepo cannot **author** those pipelines yet. Chain ingest (Alchemy → datasets → ontology) still belongs on that classic path.

Ontology-as-code types are backed by the datasources you declare in code; a **stream** can be one of them. Actions are **not** yet supported on stream-backed object types ([object edits](https://www.palantir.com/docs/foundry/object-edits/overview/)). After pipeline-backed types exist, a SuperRepo can *import* them (or point code definitions at those datasources) and own a custom app on top.

Harbor Desk wallets are **typed in**, not synced from a chain. That is a SuperRepo-demo constraint (no ingest in this product), not a production ingest design.
...
The supported SuperRepo hybrid is the other direction: `foundry import ontology` to pull existing types into a SuperRepo so the React app and functions can use them with generated types. You can also declare existing datasources (including a stream) on Ontology-as-code types; actions are not yet supported on stream-backed types.
...
**Coming** — Palantir [Coming in the future](https://www.palantir.com/docs/foundry/superrepo/in-development/):

- Data pipelines / PySpark transforms
- Compute modules
- Python functions
- External sources: SuperRepo functions calling APIs **outside the Palantir platform**
- Automate
- Agent engine / Agent SDK

**Not SuperRepo components** (Palantir does not list these as coming SuperRepo features). Workshop, AIP, and Data Connection consume the same Ontology; you do not author them in this repo.

Until pipelines and external sources land, SuperRepo is the right tool for **ontology-native writeback apps** (users create and update objects through actions) and the wrong tool to **author ingest**. It can still own the custom app and functions on types you import (or declare against existing datasources).
...
│   └── index.ts                    # Maker codegen re-exports (not the app OSDK)
├── functions/typescript-functions/
│   ├── src/lib/policy.ts           # risk weights, close status rules
│   ├── src/lib/ids.ts              # authored id, else OSDK primary key
...
“[The OSDK is generated locally and regenerated whenever those definitions change](https://www.palantir.com/docs/foundry/ontology-sdk/overview/)… without republishing an SDK version between each change.”
...
App and function code import those names from `@ontology/sdk`, which is `ontology/osdk-output`. `ontology/index.ts` re-exports Maker codegen (`./codegen/…`) for the ontology component. Do not edit `osdk-output/` or `src/generated-imports/`.
...
Harbor Desk’s queue does **not** filter server-side yet. `AppShell` calls `useOsdkObjects(investigationCase, { pageSize: 50, streamUpdates: true })`; `CaseQueue` **sorts** that page locally (open cases first, then by `riskScore`) and can hide closed rows in the UI. When the queue grows, `.where` belongs in the hook options, not in a local `.filter`. `openCase`’s `fetchPage({ $pageSize: 50 })` to detect a duplicate active case is the same demo-scale check.

### 5.3 `@osdk/react` — this is the app idiom

“[We recommend always using `@osdk/react` when building React applications with the OSDK](https://www.palantir.com/docs/foundry/ontology-sdk-react-applications/osdk-react/).” **Do not hand-roll `useState` + `useEffect` for OSDK reads.**
...
The Palantir-native way to load a case’s subject is `useLinks(item, "subjectPerson")` / `subjectOrganization`. Harbor Desk’s `SubjectPanel` and add-finding dialog traverse those named links. Hidden FKs (`personId`, `organizationId`) still exist on the object; they are the physical carrier, not what the desk reads.
...
The OSDK uses a token “[scoped only to the ontological entities you want your application to access](https://www.palantir.com/docs/foundry/ontology-sdk/overview/), in addition to the user's own permissions to the data.” That is why putting policy in functions still matters — the user might be allowed to call the action and still fail submission criteria.
...
| `finding` | Object-backed evidence on a case; open findings block close; mitigation note and resolver on resolve |
...
`riskScore` is a stored rollup of open finding weights. Palantir’s [structural guidance](https://www.palantir.com/docs/foundry/ontology/ontology-structural-guidance): values that depend on links or action-driven changes should be **derived properties**, evaluated at query time. These types are empty-backed writeback, and local preview has no derived-property pipeline, so every mutating action updates a stored copy. Named tradeoff; same as the ontology guide. `severity` on a case is a priority band, a different fact.
...
| analyst `resolvedFindings` | finding `resolvedBy` | `resolvedById` |
...
**Function-backed actions** — Harbor Desk imports `defineFunctionBackedAction` from `@osdk/maker-experimental`. `defineFunctionBackedAction({ functionApiName: "openCase" })` → `openCaseAction`. The ontology invokes the TypeScript function. The function **returns edits**. The action **applies** them. “[Only actions are allowed to mutate the Ontology's content](https://www.palantir.com/docs/foundry/superrepo/tutorial-develop-with-a-superrepo).”
...
`ontology/seed/001-harbor.mts` default-exports a `createSeed` callback. Only top-level `.mts` files in `seed/` are compiled, in sorted filename order (hence `001-`) ([advanced workflows](https://www.palantir.com/docs/foundry/superrepo/advanced-workflows/)).
...
The ontology dev server is started as `foundry start ontology --seed-dir seed`. Each **start** uses a fresh local database, seed files are re-applied, and objects edited through the local server are discarded on restart. The embedded Ontology “[is not connected to your enrollment's Ontology](https://www.palantir.com/docs/foundry/superrepo/advanced-workflows/).”

**Observed in this preview:** changing a file under `seed/` wipes and re-seeds **in process**, without restarting the ontology server (§9.2). Palantir’s pages document the fresh database on start, not that in-process path.
...
Shared rules are in `functions/typescript-functions/src/lib/policy.ts`. Risk weights: Critical 40, High 25, Medium 12, Low 5, cap 100. At 50 an `Open` case becomes `In review`. CASE-2041 seeds at **65** (Critical + High open). `statusAfterRisk` never de-escalates In review, and never touches Closed or Pending close.

Object parameters go through `objectId` in `lib/ids.ts`: authored `id` if loaded, otherwise `$primaryKey`. Action-passed instances often have the key and not the property.
...
1. Inputs are OSDK instances (the action passes objects), not raw ids you fetch by hand. `objectId` reads the authored `id`, or `$primaryKey` if the property has not loaded.
2. Validation throws [`UserFacingError`](https://www.palantir.com/docs/foundry/functions/user-facing-error/) so the UI can show the message.
3. [`createEditBatch`](https://www.palantir.com/docs/foundry/functions/typescript-v2-ontology-edits/) + `batch.update` / `batch.create` describes the write.
...
`app/index.html` has placeholders: `osdk-clientId`, `osdk-redirectUrl`, `osdk-foundryUrl`, `osdk-ontologyRid`. Marketplace remaps those meta tags on install to the target Foundry URL, OAuth client ID, redirect URL, and Ontology RID ([Install a Developer Console application with Marketplace](https://www.palantir.com/docs/foundry/developer-console/marketplace-installation/)).
...
Local mock auth has no Foundry user. Four-eyes still needs two identities, so the header identity chip stores the analyst **id string** in `sessionStorage` (`harbor-desk.actingAs` in `ActingAsContext`) and resolves the `analyst` object from the loaded list. Maya Chen is the default. Policy functions receive that object as `actingAnalyst`. The chip is the demo’s way to play both sides of four-eyes: switch analyst, then approve a close you requested.
...
Local servers mock a subset of a real Foundry instance. The embedded ontology approximates ontology behavior. It is not your enrollment. Changes in preview **never** appear on Foundry until you deploy.
...
| Ontology server | `ontology.mts` change → regenerate OSDK. `seed/` change → in-process wipe and re-seed (observed here; Palantir documents a fresh database on start, §6.4) |
...
Only listed paths are proxied. These requests use credentials from `foundry login`, not the token you used to install the CLI. If proxied calls start failing auth, `foundry login refresh` ([advanced workflows](https://www.palantir.com/docs/foundry/superrepo/advanced-workflows/)).
...
When you save `ontology.mts`, watch the `ontology:dev` pane in the `foundry start` TUI (Tab / arrows). That is codegen happening. Palantir documents the same trick in the [SuperRepo tutorial](https://www.palantir.com/docs/foundry/superrepo/tutorial-develop-with-a-superrepo).

Suggested paper trace for a new reader: `ClosePath.tsx` → `request-close-action` in `ontology.mts` → `requestClose.ts` → edits applied → case header refresh.
...
2. For deploy from outside Foundry CI: `FOUNDRY_TOKEN` set to a [user-generated token](https://www.palantir.com/docs/foundry/platform-security-third-party/user-generated-tokens/#user-generated-tokens). Those tokens have **no scope setting** ([Foundry CLI](https://www.palantir.com/docs/foundry/superrepo/foundry-cli)); the **user** needs Foundry DevOps publishing permissions ([Prepare for your first deployment](https://www.palantir.com/docs/foundry/superrepo/prepare-first-deploy/)).
...
**Palantir vs this public repo.** [Prepare for your first deployment](https://www.palantir.com/docs/foundry/superrepo/prepare-first-deploy/) and the [August 2026 announcement](https://www.palantir.com/docs/foundry/announcements/2026-08/) say: **commit `env.yml`**, because Foundry CI reads it when you deploy by tag. That is the right workflow for a private team SuperRepo aimed at a known enrollment.
...
If `env.yml` is missing, interactive `foundry deploy` can run first-run config creation (`foundry deploy --help`). `--non-interactive` (CI, or when stdin is not a TTY) disables that. `--store-rid` is a real flag on `foundry deploy` and `foundry deploy configure`; this repo’s `configure` target forwards extra args.

Pass-through example (Palantir’s example RID form):

```bash
pnpm run configure -- --store-rid ri.marketplace.main.local.example
```
...
`--project-version` expects a **semantic version** (`MAJOR.MINOR.PATCH`) that becomes the Marketplace product version ([core concepts](https://www.palantir.com/docs/foundry/superrepo/core-concepts/)). No `v` prefix. If you omit `FOUNDRY_PRODUCT_VERSION` locally, the bundle is `1.0.0` and a second deploy collides with the first.
...
If an older SuperRepo from this same folder was already installed (the original tutorial product, or a version that still had `entity`), uninstall it first or accept that upgrading the existing Marketplace installation replaces those types.
...
`--ontology-rid` is required. `--objects` is optional; when you pass it, only those object types are added or updated in the lockfile. You can also pass `--actions`, `--interfaces`, `--queries`, and `--shared-property-types`. Default output directory is `ontology/external-imports`.

Or use the **Import sidebar** of the Palantir extension for Visual Studio Code; it prints the same command ([tutorial](https://www.palantir.com/docs/foundry/superrepo/tutorial-develop-with-a-superrepo)).

That writes `ontology/external-imports/ontology-full-metadata.json` (commit this lockfile — [codegen depends on it](https://www.palantir.com/docs/foundry/superrepo/core-concepts/)). Codegen produces stubs under `ontology/src/generated-imports/`. Reference those from `ontology.mts`, then add links, actions, functions, and UI around them.
...
For a production investigation desk, the sound hybrid is: **classic Foundry owns ingest + pipelines + object types**; a SuperRepo **imports** those types (or declares the existing datasources in code) and owns the custom app/functions. Do not let SuperRepo create `Wallet` as empty-backed writeback if a pipeline needs to back it in the UI — the lock will fight you. Harbor Desk’s wallets are writeback on purpose, so the SuperRepo can demo without ingest.
...
- Functions cannot call APIs outside the Palantir platform yet (SuperRepo external sources).
...
| **Singleton** | `installMode: SINGLETON`: Palantir’s default; deployed once per Ontology |
